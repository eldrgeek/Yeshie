...
// --- The Main Component ---
export const SpeechInput = forwardRef(({
    ...
    // Use forwarded ref or create a local one if not provided
    const localTextAreaRef = useRef<HTMLTextAreaElement>(null);
    const textAreaRef = (ref as React.RefObject<HTMLTextAreaElement>) || localTextAreaRef;
    ...
    }, [isListening, isSupported, permissionStatus, addToLog, isTranscribing, isTextareaFocused]);
    ...
    }, [isSupported, permissionStatus, addToLog, startListening, stopListening, isListening]); // Run only on support/permission change
    ...
    const handleTextareaFocus = () => {
        ...
        // If transcribing is enabled and we have permission, ensure recognition is running
        if (isTranscribing && isSupported && permissionStatus === 'granted' && !isListening) {
            wasListeningIntentionallyRef.current = true;
            startListening();
        }
    };
