...
import {
  rememberCurrentTab,
  attemptTabFocusWithRetries,
  storedTabId,
} from "../functions/tabFocus";
...
rememberCurrentTab().then((tabId) => {
  if (tabId) {
    logInfo(
      "YeshieContent",
      `Tab ID ${tabId} will be restored after extension loads`,
    );
  }
});
...
const isMatchingURL = (pattern: string) => {
  ...
  const currentMatch =
    currentURL.hostname === patternURL.hostname &&
    currentURL.pathname.startsWith(patternURL.pathname);
  const cordMatch =
    currentURL.hostname === cordURL.hostname &&
    currentURL.pathname.startsWith(cordURL.pathname);
...
const Yeshie: React.FC = () => {
  ...
  useEffect(() => {
    storageGet<boolean>(isOpenKey)
      .then((value) => {
        setIsOpen(value ?? false);
        logInfo("YeshieContent", "storage_get: isOpenKey", {
          key: isOpenKey,
      ...
      .catch((error) => {
        logError(
          "YeshieContent",
          `Error getting initial state for ${isOpenKey}`,
          { error },
        );
        const errorMessage =
          error instanceof Error ? error.message : String(error);
        logError("YeshieContent", "storage_error: getIsOpenInitial", {
          operation: "getIsOpenInitial",
          key: isOpenKey,
          error: errorMessage,
        });
        setIsOpen(false);
  ...
  const updateIsOpen = useCallback(
    async (newIsOpen: boolean) => {
      ...
      try {
        ...
        logInfo("YeshieContent", "storage_set: isOpenKey", {
          key: isOpenKey,
          value: newIsOpen,
        });
      } catch (error) {
        logError("YeshieContent", `Error setting state for ${isOpenKey}`, {
          error,
        });
        const errorMessage =
          error instanceof Error ? error.message : String(error);
        logError("YeshieContent", "storage_error: setIsOpen", {
          operation: "setIsOpen",
          key: isOpenKey,
          error: errorMessage,
        });
  ...
  const updateContext = useCallback(
    async (newContextPart: Partial<TabContext>) => {
      if (tabId === null) {
      ...
      setContext((prevContext) => {
        const currentContext = prevContext || {
        ...
        (async () => {
          try {
            ...
            logInfo("YeshieContent", "storage_set: contextKey", {
              key: contextKey,
              tabId: tabId,
            });
            logInfo(
              "YeshieContent",
              `Context updated and saved for tab ${tabId}`,
              { context: updated },
            );
          } catch (error) {
            logError(
              "YeshieContent",
              `Failed to save context for tab ${tabId}`,
              { error },
            );
            const errorMessage =
              error instanceof Error ? error.message : String(error);
            logError("YeshieContent", "storage_error: updateContext", {
  ...
  const debouncedUpdateContext = useCallback(
    debounce(
      (newContext: Partial<TabContext>) => updateContext(newContext),
      1000,
    ),
    [updateContext],
  );
  ...
  const handleMessage = useCallback(async (event: MessageEvent) => {
    if (event.data && event.data.type === "command") {
      logInfo("YeshieContent", "Received command from YeshieEditor", {
        commandData: event.data,
      });
      try {
  ...
  useEffect(() => {
    if (
      ...
      !document.getElementById(getShadowHostId())
    ) {
      logInfo(
        "YeshieContent",
        "Yeshie is in an iframe or shadow host not found, not rendering",
      );
      return;
    }
    ...
    async function init() {
      ...
      logInfo("YeshieContent", "Initializing Yeshie content script", {
        isOpen,
        isReady,
      });
      try {
        ...
        if (response && typeof response.tabId === "number") {
          ...
          logInfo("YeshieContent", "storage_get: contextKey", {
            key: contextKey,
            found: storedContext !== undefined,
          });
          ...
          if (storedContext) {
            ...
            logInfo("YeshieContent", "storage_set: contextKey, Initializing", {
              key: contextKey,
              reason: "Initializing new context",
            });
        ...
        logError(
          "YeshieContent",
          "Error during init (getTabId or context handling)",
          { error },
        );
        const errorMessage =
          error instanceof Error ? error.message : String(error);
        logError("YeshieContent", "init_error: getTabId/context", {
          step: "getTabId/context",
          error: errorMessage,
        });
        setConnectionError(true);
        const msg = (errorMessage || "").toLowerCase();
        if (msg.includes("context") && msg.includes("invalid")) {
          chrome.runtime.sendMessage({ type: "CONTEXT_INVALIDATED" });
        } else {
          chrome.runtime.sendMessage({
            type: "CONTENT_SCRIPT_CONNECTION_ERROR",
          });
        }
      }
    }
    ...
    const observer = new MutationObserver((mutations, obs) => {
      ...
      if (targetElement) {
        ...
        logInfo(
          "YeshieContent",
          "Target element found, Yeshie is ready to render",
        );
      }
    });
  ...
  useEffect(() => {
    if (isReady && storedTabId) {
      console.log(
        "Extension is ready, attempting to restore tab focus to:",
        storedTabId,
      );
    ...
    if (isReady) {
      storageGetAll()
        .then((allData) => {
          ...
          logInfo("YeshieContent", "storage_get_all: report_count", {
            purpose: "report_count",
        ...
        .catch((error) => {
          ...
          const errorMessage =
            error instanceof Error ? error.message : String(error);
          logError("YeshieContent", "storage_error: getReportCount", {
      ...
      >
        ...
        >
          ...
          <span style={{ marginLeft: "12px" }}>
            Session ID: {sessionID !== null ? sessionID : "Loading..."}
          </span>
        </div>
      </div>
