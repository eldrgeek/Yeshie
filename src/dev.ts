...
const jobs: Job[] = [
  {
    name: "server",
    do: "nodemon --watch src -e ts,tsx --exec ts-node src/server.ts",
  },
  { name: "client", do: "cd client && npm run dev" },
  {
    name: "listener",
    do: "nodemon --watch src -e py --exec python src/listener.py",
  },
  {
    name: "monitor",
    do: "nodemon --watch src -e py --exec python src/monitor.py",
  },
  {
    name: "testkb",
    do: "nodemon --watch src -e py --exec python src/testkb.py",
  },
  {
    name: "winmonitor",
    do: "nodemon --watch src -e py --exec python src/winmonitor.py",
  },
  { name: "extension", do: "cd extension && npm run dev" },
  {
    name: "messages",
    do: "nodemon -watch scripts/messages.sh --exec ./scripts/messages.sh",
  },
  {
    name: "llm",
    do: "nodemon --watch src -e py --exec python src/llmserver.py",
  },
  {
    name: "codeStore",
    do: "nodemon --watch src/codeStore.py -e py --exec python src/codeStore.py",
  },
];
...
function main() {
  ...
  if (profile) {
    if (profile in profiles) {
      ...
      console.error(
        `Profile "${profile}" not found. Available profiles: ${Object.keys(profiles).join(", ")}`,
      );
      process.exit(1);
    }
  } else {
