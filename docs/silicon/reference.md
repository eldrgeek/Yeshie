...
| `assess_state` | Judge the page against a state graph (the step's `stateGraph`, else the payload's) and compare the state with `expect.state`. A mismatch routes to `onMismatch: "branch:<name>"`, naming a branch in the payload's `branches`. Signals: `url_matches`, `url_not_matches`, `element_visible`, `element_absent`, `element_text` (the selector's text contains `text`), `attribute_change`. A node matches when all of its signals do, and the first matching node wins. **The nodes are judged in alphabetical order, not the order written** — measured live 2026-09-15, when a graph of `groups-available` and `groups-unavailable` always answered `groups-available`, although the empty-state node was written first. So write a guard as a single node, or give the nodes signals that cannot both hold. |
